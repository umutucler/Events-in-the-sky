...
// Don't duplicate me!
	...
	function avada_woocommerce_before_cart_table( $args ) {
		...
		if ( $woocommerce->cart->get_cart_contents_count() == 1 ) {
			$html .= '<h2>' . sprintf( __( 'Du har %d vare i kurven', 'Avada' ), $woocommerce->cart->get_cart_contents_count() ) . '</h2>';
		} else {
			$html .= '<h2>' . sprintf( __( 'Du har %d varer i kurven', 'Avada' ), $woocommerce->cart->get_cart_contents_count() ) . '</h2>';
		}
			...
			<div class="avada-shipping-calculator-form">
				...
				<?php if ( apply_filters( 'woocommerce_shipping_calculator_enable_postcode', true ) ) : ?>
					...
					<div class="form-row form-row-wide fusion-layout-column fusion-one-half fusion-spacing-yes fusion-column-last">
						<input type="text" class="input-text" value="<?php echo esc_attr( WC()->customer->get_shipping_postcode() ); ?>" placeholder="<?php esc_attr_e( 'Postnr.', 'woocommerce' ); ?>" name="calc_shipping_postcode" id="calc_shipping_postcode" />
					</div>
